...
git clone https://github.com/athertahir/generative-adversarial-networks.git

# copy the script into the container; note: the resulting file is owned by root and it not executable
docker cp ~/tutorial/generative-adversarial-networks/prepareContainer.sh jupyter:/tmp/prepareContainerRoot.sh

docker cp  ~/tutorial/generative-adversarial-networks/chapter_02/. jupyter:/home/jovyan/work

# copy the script to make it accessible and executable 
docker exec -it jupyter bash -c 'sudo chmod -R 777 ~ && cp /tmp/prepareContainerRoot.sh ~/prepareContainer.sh && chmod +x ~/prepareContainer.sh && ~/prepareContainer.sh && conda install pydotplus graphviz python-graphviz -y'
